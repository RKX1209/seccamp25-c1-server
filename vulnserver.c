...
void draw_and_send_maze(int client_fd, char maze[MAZE_H][MAZE_W+1], const char *name, int px, int py) {
    ...
    for (int y = 0; y < MAZE_H && len < (int)sizeof(out) - 1; ++y) {
        for (int x = 0; x < MAZE_W && len < (int)sizeof(out) - 1; ++x) out[len++] = (x==px && y==py) ? '@' : maze[y][x];
        out[len++] = '\n';
    }
    out[len++] = '\n'; out[len] = '\0'; send_all(client_fd, out, (size_t)len);
}

int main(void) {
    int server_fd = socket(AF_INET, SOCK_STREAM, 0); if (server_fd < 0) { perror("socket"); return 1; }
    ...
    while (1) {
        ...
        char linebuf[MAX_LEN]; ssize_t n = recv_line(cfd, linebuf, sizeof(linebuf)); if (n <= 0) { close(cfd); continue; }

        char player_name[NAME_MAX]; strncpy(player_name, linebuf, NAME_MAX-1); player_name[NAME_MAX-1] = 0;
